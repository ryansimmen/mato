...
func usage() {
	fmt.Fprintf(os.Stderr, `Usage: mato [--repo <path>] [--branch <name>] [copilot-args...]
...
func main() {
	repoRoot, branch, copilotArgs, err := parseArgs(os.Args[1:])
	if err == errHelp {
		usage()
		os.Exit(0)
	}
	...
	if err := run(repoRoot, branch, copilotArgs); err != nil {
		fmt.Fprintf(os.Stderr, "mato error: %v\n", err)
		os.Exit(1)
	}
}
...
func parseArgs(args []string) (string, string, []string, error) {
	...
	return repoRoot, branch, copilotArgs, nil
}

...
9. Resolve host tools and runtime dependencies: `copilot`, `git`, `git-upload-pack`, `git-receive-pack`, `gh`, optional `gopls`, `GOROOT`, optional `~/.config/gh`, optional `/etc/ssl/certs`, and Git author/committer identity. At the same time, `mato` prepares GitHub authentication for Docker launches: explicit `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, and `GITHUB_TOKEN` env vars are forwarded into containers unchanged, and when none are set it makes a best-effort `gh auth token` lookup so Copilot sessions that rely on host-side credential storage still work inside containers. Docker image, task model, review model, task reasoning effort, review reasoning effort, agent timeout, and retry cooldown are already resolved in `cmd/mato/main.go`.
...
- `recoverStuckTask(...)` runs immediately after `runOnce(...)` returns: if the task file is still in `in-progress/`, the host first checks whether the work branch was already pushed. Safe pushed-task handoffs are reconstructed into `ready-for-review/`; pushed tasks whose handoff metadata is missing, corrupt, or otherwise insufficient are quarantined to `failed/` with a terminal-failure marker instead of being left stranded or requeued as fresh work. Only confirmed pre-push work is returned to `backlog/` with a normal failure record.
- Merge processing happens after any agent run in the same outer loop.
- Runtime sidecar data lives under `.mato/runtime/`. Work launches, work pushes, and review launches/completions update `.mato/runtime/taskstate/<task-filename>.json` best-effort so follow-up reviews can carry explicit host-curated context even before durable Copilot session resume exists.
...
- `queue.RecoverOrphanedTasks(...)` (in `queue.go`) scans `in-progress/*.md`; if the claiming agent is no longer active, it uses the same pushed-task handoff policy as `recoverStuckTask(...)`: safe pushed work is reconstructed into `ready-for-review/`, unrecoverable pushed handoffs are moved to `failed/` with a terminal-failure marker, and only pre-push work is returned to `backlog/` with `<!-- failure: mato-recovery ... -->`.
...
Immediately after claim handoff, the host records a best-effort `work-launched` taskstate with the claimed task branch and target branch so pre-push setup failures (clone creation, branch setup, starting-tip capture) remain safely requeueable. Once branch setup succeeds and the starting tip is known, `runOnce(...)` refreshes that same taskstate with the captured `last_head_sha`. After the agent exits, the host (`postAgentPush` in `task.go`) checks for commits on the task branch. If commits exist and the task is still in `in-progress/`, the host pushes the branch with `--force-with-lease`, writes the `<!-- branch: ... -->` marker, moves the task to `ready-for-review/`, updates `.mato/runtime/taskstate/<task-filename>.json` with the pushed branch tip and outcome, updates the `work` session's `last_head_sha`, and sends `conflict-warning` and `completion` messages. That branch marker is the durable handoff record for all post-work phases; if a `ready-for-review/` or `ready-to-merge/` task is missing it, the host records a failure/requeue instead of synthesizing a branch from the filename. When orphan or same-process recovery encounters already-pushed work, it reconstructs `ready-for-review/` only when the handoff metadata is trustworthy; otherwise it moves the task to `failed/` with a `<!-- terminal-failure: ... -->` marker instead of leaving it stranded in `in-progress/` or downgrading it back to `backlog/`.
...
waiting/ --deps met--> backlog/ --host claim--> in-progress/ --host push--> ready-for-review/ --approved--> ready-to-merge/ --merge--> completed/
   ...
   +--dep not met---------+                          +--host recovery (pre-push)--> backlog/
                          |                          +--host recovery (safe pushed handoff)--> ready-for-review/
                          |                          +--host recovery (unrecoverable pushed handoff)--> failed/
...
### How it works
1. `selectAndLockReview(tasksDir, idx)` walks the review candidates in the earlier `PollIndex` ordering, acquires the per-task review lock, then re-reads the locked task from disk before launching review. Ordering may come from the stale snapshot, but the final go/no-go decision uses the current on-disk `<!-- review-failure: ... -->` count and `<!-- branch: ... -->` marker.
2. If that lock-time revalidation finds the review retry budget exhausted, the host appends a `<!-- terminal-failure: ... -->` marker and moves the task to `failed/`. If the branch marker is now missing, the host writes a `<!-- review-failure: ... -->` record and skips the review.
3. The host verifies the task branch exists (`git rev-parse --verify`). If the branch is missing, the host writes a `<!-- review-failure: ... -->` record and skips the review.
4. `runReview(...)` launches a review agent in the same Docker container model as task agents (ephemeral container, temp clone, identical bind mounts). Before launch, the host resolves the current branch tip, loads any existing `.mato/runtime/taskstate/<task-filename>.json`, reads the most recent review-rejection reason from the task file, renders a `Review context:` block into the embedded review prompt, and, after clone setup succeeds, records a best-effort `review-launched` taskstate update. When review-session resume is enabled, the host also loads or creates a durable `review` session record and passes `copilot --resume=<session-id>` to the review run.
5. The review agent diffs the task branch against the target branch, analyzes the changes, and writes a JSON verdict file to `.mato/messages/verdict-<filename>.json` with `{"verdict":"approve"}`, `{"verdict":"reject","reason":"..."}`, or `{"verdict":"error","reason":"..."}`.
6. After the review agent exits, the host reads the verdict file via `postReviewAction(...)`:

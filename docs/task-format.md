...
| --- | --- | --- | --- |
| `id` | string | filename without `.md` | Stable task ID. If omitted, `my-task.md` becomes `my-task`. Use this in `depends_on`. Completed deps match either explicit `id` or filename stem, and dependency-context lookup follows that same alias rule when loading prerequisite completion details. Unknown frontmatter keys are ignored. |

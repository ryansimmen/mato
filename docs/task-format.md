...
## Frontmatter Fields
Supported keys come from `TaskMeta`. Unknown top-level keys are rejected at parse time.
...
| --- | --- | --- | --- |
| `id` | string | filename without `.md` | Stable task ID. If omitted, `my-task.md` becomes `my-task`. Use this in `depends_on`. Completed deps match either explicit `id` or filename stem, and dependency-context lookup follows that same alias rule when loading prerequisite completion details. Unknown top-level frontmatter keys are rejected so typos fail fast. |
| `priority` | int | `50` | Lower numbers are higher priority. Omitting the field, leaving it as an empty scalar (`priority:`), or setting it to YAML `null` all resolve to the default `50`; an explicit `0` is preserved. The host derives claim order from the effective runnable backlog, sorted by priority ascending and then filename ascending; `.queue` exports that same derived order for inspection. |
...
| Field | Type | Default | Reference |
| --- | --- | --- | --- |
| `max_retries` | int | `3` | Maximum number of `<!-- failure: ... -->` records before the task moves to `failed/`. Omitting the field, leaving it as an empty scalar (`max_retries:`), or setting it to YAML `null` all resolve to the default `3`; an explicit `0` is preserved. Must be a non-negative integer (≥ 0); negative values are rejected at parse time. A task with `max_retries: 3` is moved to `failed/` once it accumulates 3 failure records (i.e. `failures >= max_retries`). The host merge queue reads this per-task from frontmatter (authoritative). The agent receives the same resolved value via `MATO_MAX_RETRIES` as a safety net. |
...
The same defaults apply when `priority` or `max_retries` are written as empty scalars or explicit YAML `null`.

...
As of `gh` v2.96.0, `gh skill` has no uninstall/remove command. Use `gh skill list --scope user --json skillName,path,agentHosts` to find user-scoped installations, then remove the `mato` directory for your agent host. Common locations include:
...
Each release publishes a `*.intoto.jsonl` SLSA build provenance bundle, per-archive cosign `.sigstore.json` bundles, a signed `checksums.txt`, and per-archive [SPDX 2.3](https://spdx.dev/) SBOMs. The install script verifies the archive checksum and, when `cosign` is installed, the archive and checksum signatures. The steps below let you verify a manually-downloaded archive.
...
The release workflow also registers these attestations with GitHub, where the `gh attestation verify` command above fetches them by default. To verify using the downloaded provenance bundle instead, pass it with `--bundle`.

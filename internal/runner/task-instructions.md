...
printf '{"id":"%s-%s-verify-claim-%s","from":"%s","type":"progress","task":"%s","branch":"%s","body":"Step: VERIFY_CLAIM","sent_at":"%s"}\n' "$MATO_TS" "$AGENT_ID" "$$" "$AGENT_ID" "$FILENAME" "$BRANCH" "$MATO_SENT_AT" > "MESSAGES_DIR_PLACEHOLDER/events/${MATO_NONCE}-${AGENT_ID}-verify-claim.json" || true
...
date -u +%Y-%m-%dT%H:%M:%SZ > /tmp/mato-sent-at-$$.txt
read MATO_SENT_AT < /tmp/mato-sent-at-$$.txt
printf '{"id":"%s-%s-work-%s","from":"%s","type":"progress","task":"%s","branch":"%s","body":"Step: WORK","sent_at":"%s"}\n' "$MATO_TS" "$AGENT_ID" "$$" "$AGENT_ID" "$FILENAME" "$BRANCH" "$MATO_SENT_AT" > "MESSAGES_DIR_PLACEHOLDER/events/${MATO_NONCE}-${AGENT_ID}-work.json" || true
cat "$TASK_PATH"
...
date -u +%Y-%m-%dT%H:%M:%SZ > /tmp/mato-sent-at-$$.txt
read MATO_SENT_AT < /tmp/mato-sent-at-$$.txt
printf '{"id":"%s-%s-commit-%s","from":"%s","type":"progress","task":"%s","branch":"%s","body":"Step: COMMIT","sent_at":"%s"}\n' "$MATO_TS" "$AGENT_ID" "$$" "$AGENT_ID" "$FILENAME" "$BRANCH" "$MATO_SENT_AT" > "MESSAGES_DIR_PLACEHOLDER/events/${MATO_NONCE}-${AGENT_ID}-commit.json" || true
git status --short
git add -A

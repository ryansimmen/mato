...
MATO_NONCE="${MATO_TS}-$$"
date -u +'{"id":"'"$MATO_TS"'-'"$AGENT_ID"'-verify-review","from":"'"$AGENT_ID"'","type":"progress","task":"'"$FILENAME"'","branch":"'"$BRANCH"'","body":"Step: VERIFY_REVIEW","sent_at":"%Y-%m-%dT%H:%M:%SZ"}' > "MESSAGES_DIR_PLACEHOLDER/events/${MATO_NONCE}-${AGENT_ID}-verify-review.json" || true
```

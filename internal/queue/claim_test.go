...
func setupClaimTestDir(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	for _, sub := range append(dirs.All, dirs.Locks) {
...
func TestSelectAndClaimTask_Normal(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "alpha.md"), "# Alpha\nDo alpha.\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "beta.md"), "# Beta\nDo beta.\n")
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.InProgress, "alpha.md")); err != nil {
		t.Fatalf("task not in in-progress: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "alpha.md")); !os.IsNotExist(err) {
		t.Fatal("task still in backlog after claim")
	}

	// Check claimed-by header
	data, _ := os.ReadFile(filepath.Join(dir, dirs.InProgress, "alpha.md"))
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "beta.md")); err != nil {
...
func TestSelectAndClaimTask_RetryExhausted(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "retry.md"), strings.Join([]string{
		"# Retry task",
		"<!-- failure: one -->",
		"<!-- failure: two -->",
		"<!-- failure: three -->",
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Failed, "retry.md")); err != nil {
		t.Fatalf("task not in failed: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "retry.md")); !os.IsNotExist(err) {
...
func TestSelectAndClaimTask_RetryExhausted_PreservesVerdictFallback(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, filename), strings.Join([]string{
		"# Verdict retry task",
	...
	// Task should be in failed/.
	if _, err := os.Stat(filepath.Join(dir, dirs.Failed, filename)); err != nil {
		t.Fatalf("task not in failed: %v", err)
	}
...
func TestSelectAndClaimTask_SkipsExhaustedClaimsNext(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "good.md"), "# Good\nDo it.\n")
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Failed, "bad.md")); err != nil {
...
func TestSelectAndClaimTask_InvalidCandidateNamesSkipped(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "real.md"), "# Real\n")
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "real.md")); !os.IsNotExist(err) {
...
func TestSelectAndClaimTask_DeferredExclusion(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "high.md"), "# High\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "low.md"), "# Low\n")
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "high.md")); err != nil {
...
func TestSelectAndClaimTask_DemotesDependencyBlockedBacklogTask(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "blocked.md"), "---\nid: blocked\ndepends_on: [missing]\n---\n# Blocked\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "runnable.md"), "# Runnable\n")
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Waiting, "blocked.md")); err != nil {
		t.Fatalf("blocked.md should be moved to waiting/: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "blocked.md")); !os.IsNotExist(err) {
...
func TestSelectAndClaimTask_FreshlyEditedDependencyBlockedTaskIsDemoted(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "edited.md"), "# Edited\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "fallback.md"), "# Fallback\n")

	idx := BuildIndex(dir)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "edited.md"), "---\nid: edited\ndepends_on: [missing]\n---\n# Edited\n")
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Waiting, "edited.md")); err != nil {
...
func TestSelectAndClaimTask_CandidateOrdering(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "z-last.md"), "# Z Last\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "a-first.md"), "# A First\n")
...
func TestOrderedRunnableFilenames_UsesRunnableViewOrder(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "z-last.md"), "---\npriority: 20\n---\n# Z Last\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "a-first.md"), "---\npriority: 20\n---\n# A First\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "top.md"), "---\npriority: 5\n---\n# Top\n")
...
func TestClaimSelection_IndexedMatchesFilesystemFallback(t *testing.T) {
	setup := func(t *testing.T, dir string) {
		t.Helper()
		testutil.WriteFile(t, filepath.Join(dir, dirs.InProgress, "active.md"), "<!-- branch: task/add-feature -->\n# Active\n")
		testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "add feature.md"), "---\npriority: 5\n---\n# Add Feature\n")
		testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "later.md"), "---\npriority: 20\n---\n# Later\n")
	}
...
func TestSelectAndClaimTask_FrontmatterMaxRetries(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "custom.md"), strings.Join([]string{
		"---",
		"max_retries: 1",
		"---",
		"# Custom retries",
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Failed, "custom.md")); err != nil {
...
func TestSelectAndClaimTask_ClaimedByWriteFailure_FallsBack(t *testing.T) {
	...
	if err := os.Chmod(filepath.Join(dir, dirs.Backlog, "broken.md"), 0o000); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.InProgress, "fallback.md")); err != nil {
...
func TestSelectAndClaimTask_ClaimedByWriteFailure_AllFail(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "only.md"), "# Only\nDo only.\n")

	// Make the file unreadable so prependClaimedBy fails.
	if err := os.Chmod(filepath.Join(dir, dirs.Backlog, "only.md"), 0o000); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "only.md")); err != nil {
		t.Fatalf("only.md should be back in backlog: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, dirs.InProgress, "only.md")); !os.IsNotExist(err) {
...
func TestSelectAndClaimTask_ZeroMaxRetries(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "zero-retry.md"), strings.Join([]string{
		"---",
		"max_retries: 0",
		"---",
		"# Zero retries",
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Failed, "zero-retry.md")); err != nil {
...
func TestSelectAndClaimTask_RollbackFailure_ReturnsError(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "stuck.md"), "# Stuck\nDo stuck.\n")
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.InProgress, "stuck.md")); err != nil {
...
func TestSelectAndClaimTask_RollbackFailure_SkipsFurtherCandidates(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "first.md"), "# First\nDo first.\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "second.md"), "# Second\nDo second.\n")
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "second.md")); err != nil {
...
func TestSelectAndClaimTask_PrependFails_RollbackSucceeds_ContinuesToNext(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "broken.md"), "# Broken\nDo broken.\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "healthy.md"), "# Healthy\nDo healthy.\n")
...
func TestSelectAndClaimTask_RetryExhausted_MoveToFailedFails_RollbackToBacklog(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "exhausted.md"), strings.Join([]string{
		"# Exhausted",
		"<!-- failure: one -->",
		"<!-- failure: two -->",
		"<!-- failure: three -->",
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "exhausted.md")); err != nil {
		t.Fatalf("exhausted.md should be back in backlog: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, dirs.InProgress, "exhausted.md")); !os.IsNotExist(err) {
		t.Fatal("exhausted.md should NOT be in in-progress")
	}

	// healthy.md should still be in backlog (not attempted after hard error)
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "healthy.md")); err != nil {
...
func TestSelectAndClaimTask_RetryExhausted_DoubleFailure_ReturnsError(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "stuck.md"), strings.Join([]string{
		"# Stuck",
		"<!-- failure: one -->",
		"<!-- failure: two -->",
		"<!-- failure: three -->",
...
func TestRecoverOrphanedTasks_HandlesStrandedWithoutClaimedBy(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "orphan.md")); err != nil {
		t.Fatalf("orphan.md should be recovered to backlog: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, dirs.InProgress, "orphan.md")); !os.IsNotExist(err) {
		t.Fatal("orphan.md should no longer be in in-progress")
	}

	// Verify a failure record was appended
	data, err := os.ReadFile(filepath.Join(dir, dirs.Backlog, "orphan.md"))
	if err != nil {
...
func TestSelectAndClaimTask_InvalidYAML_Skipped(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "bad-yaml.md"), strings.Join([]string{
		"---",
		"priority: [invalid",
		"---",
		"# Bad YAML task",
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "bad-yaml.md")); err != nil {
...
func TestSelectAndClaimTask_InvalidYAML_ExhaustedRetries_Skipped(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "bad-exhausted.md"), strings.Join([]string{
		"---",
		"max_retries: !!invalid",
		"---",
		"# Exhausted bad YAML",
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Failed, "bad-exhausted.md")); !os.IsNotExist(err) {
		t.Fatalf("malformed task should not be moved to failed by claim path: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "bad-exhausted.md")); err != nil {
		t.Fatalf("malformed task should remain in backlog until reconciled: %v", err)
	}
}
...
func TestSelectAndClaimTask_UnreadableFile_Skipped(t *testing.T) {
	...
	taskPath := filepath.Join(dir, dirs.Backlog, "unreadable.md")
...
func TestSelectAndClaimTask_BranchCollisionAddsDisambiguator(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, dirs.InProgress, "add-feature.md"), inProgressContent)

	// Create a new backlog task whose sanitized name also resolves to
	// "task/add-feature" (spaces become dashes).
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "add feature.md"), "# Add Feature (v2)\n")
...
func TestSelectAndClaimTask_ReusesExistingBranchMarker(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "retry.md"), content)
...
func TestSelectAndClaimTask_NewTaskDoesNotSetRecordedBranchFlag(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "fresh.md"), "# Fresh\n")
...
func TestSelectAndClaimTask_RewritesCollidingExistingBranchMarker(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.ReadyReview, "active.md"), "<!-- branch: task/existing-branch -->\n# Active\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "retry.md"), "<!-- branch: task/existing-branch -->\n# Retry\n")
...
func TestSelectAndClaimTask_BranchMarkerWriteFailureRollsBackToBacklog(t *testing.T) {
	dir := setupClaimTestDir(t)
	taskPath := filepath.Join(dir, dirs.Backlog, "retry.md")
	original := "# Retry\n"
	testutil.WriteFile(t, taskPath, original)
	...
	claimWriteFileFn = func(path string, data []byte) error {
		if path == filepath.Join(dir, dirs.InProgress, "retry.md") && strings.Contains(string(data), "<!-- branch:") {
	...
	mustBacklog := filepath.Join(dir, dirs.Backlog, "retry.md")
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.InProgress, "retry.md")); !os.IsNotExist(err) {
...
func TestSelectAndClaimTask_BranchCollisionFromMalformedActiveTask(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, dirs.ReadyReview, "broken.md"), broken)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "add feature.md"), "# Add Feature (v2)\n")
...
func TestSelectAndClaimTask_NoBranchCollision_NormalBranch(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "unique-task.md"), "# Unique Task\n")
...
func TestCollectActiveBranches(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, dirs.InProgress, "a.md"), "<!-- branch: task/alpha -->\n# A\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.InProgress, "b.md"), "<!-- branch: task/beta -->\n# B\n")

	// One without a branch comment.
	testutil.WriteFile(t, filepath.Join(dir, dirs.InProgress, "c.md"), "# C (no branch)\n")

	// Tasks in ready-for-review and ready-to-merge should also be found.
	testutil.WriteFile(t, filepath.Join(dir, dirs.ReadyReview, "d.md"), "<!-- branch: task/delta -->\n# D\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.ReadyMerge, "e.md"), "<!-- branch: task/epsilon -->\n# E\n")
...
func TestSelectAndClaimTask_DestinationExistsInProgress(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "dup.md"), "# Dup\nDo dup.\n")
	// Pre-existing file in in-progress/ with same name.
	testutil.WriteFile(t, filepath.Join(dir, dirs.InProgress, "dup.md"),
		"<!-- claimed-by: other -->\n# Dup\nAlready claimed.\n")
	...
	data, _ := os.ReadFile(filepath.Join(dir, dirs.InProgress, "dup.md"))
...
func TestSelectAndClaimTask_DestinationExistsInFailed(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, dirs.Failed, "old.md"), "# Old task (original)\n")
	...
	data, _ := os.ReadFile(filepath.Join(dir, dirs.Failed, "old.md"))
	if !strings.Contains(string(data), "original") {
		t.Fatal("failed/old.md was overwritten")
	}

	// Task should be rolled back to backlog.
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "old.md")); err != nil {
		t.Fatalf("old.md should be back in backlog: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, dirs.InProgress, "old.md")); !os.IsNotExist(err) {
...
func TestSelectAndClaimTask_RollbackDestinationExists(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "race.md"), "# Race\n")
	...
	claimPrependFn = func(path, agentID, claimedAt string) error {
		testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "race.md"), "# Race (reappeared)\n")
		return fmt.Errorf("simulated prepend failure")
	}
	...
	data, _ := os.ReadFile(filepath.Join(dir, dirs.Backlog, "race.md"))
...
func TestOrderedRunnableFilenames_ExcludesNames(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "keep.md"), "---\npriority: 10\n---\n# Keep\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "skip.md"), "---\npriority: 5\n---\n# Skip\n")
...
func TestSelectAndClaimTask_CandidateOrderPreservesOrder(t *testing.T) {
	dir := setupClaimTestDir(t)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "a-first.md"), "# A\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "b-second.md"), "# B\n")
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "c-third.md"), "# C\n")
...
func TestSelectAndClaimTask_RecentFailure_Skipped(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "hot.md"), strings.Join([]string{
		"# Hot task",
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "hot.md")); err != nil {
...
func TestSelectAndClaimTask_OldFailure_Claimed(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "cold.md"), strings.Join([]string{
		"# Cold task",
...
func TestSelectAndClaimTask_NoFailures_ClaimedImmediately(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "fresh.md"), "# Fresh task\nDo stuff.\n")
...
func TestSelectAndClaimTask_RecentFailure_SkipsToNext(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "aaa-hot.md"), strings.Join([]string{
		"# Hot task",
		fmt.Sprintf("<!-- failure: agent-1 at %s step=WORK error=crash -->", recentTS),
		"",
	}, "\n"))
	// Second task has no failures — should be claimed.
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "bbb-ok.md"), "# OK task\nDo stuff.\n")
	...
	if _, err := os.Stat(filepath.Join(dir, dirs.Backlog, "aaa-hot.md")); err != nil {
...
func TestSelectAndClaimTask_FencedCodeFailure_NoCooldown(t *testing.T) {
	...
	recentTS := time.Now().UTC().Add(-30 * time.Second).Format(time.RFC3339)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "fenced.md"), strings.Join([]string{
		"# Fenced task",
		"```",
		fmt.Sprintf("<!-- failure: agent-1 at %s step=WORK error=crash -->", recentTS),
		"```",
...
func TestSelectAndClaimTask_ProseFailure_NoCooldown(t *testing.T) {
	...
	recentTS := time.Now().UTC().Add(-30 * time.Second).Format(time.RFC3339)
	testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "prose.md"), strings.Join([]string{
		"# Prose task",
...
func TestHasClaimableBacklogTask(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			setup: func(t *testing.T, dir string) {
				testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "blocked.md"), strings.Join([]string{
					"---",
		...
		},
		{
			...
			setup: func(t *testing.T, dir string) {
				testutil.WriteFile(t, filepath.Join(dir, dirs.Backlog, "skip.md"), "# Skip\n")
			},
			want: false,
		},
	}
